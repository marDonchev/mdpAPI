...
const Api = class {
	...
	constructor(settings) {
		...
		this.logStyle = "background: #1890ff; color: white; padding: 2px 5px; border-radius: 2px";
		this.logPrefix = "API >";
		...
		if (settings.unauthorized_callback) this.unauthorized_callback = settings.unauthorized_callback;
		...
		if (typeof settings.logStyle !== "undefined") this.logStyle = String(settings.logStyle);
		// Details settings -----
	...
	async delete(_command, _body, _successCallback, _errorCallback) {
		//console.log("API Delete _command", _command);
		this._log(`DELETE > ${_command}`);
		return this.call("DELETE", _command, _body, _successCallback, _errorCallback);
	}
			...
			switch (this.fetchLibName) {
				...
				default:
					if (_method.toUpperCase() === "GET" && typeof _body !== "undefined") {
						...
						settingsObject["body"] = typeof _body !== "undefined" ? JSON.stringify(_body) : null;
					}
						...
						.then((res) => {
							...
							if (resStatus >= 200 && resStatus < 300) {
								...
								if (!_successCallback) {
									...
									//
									if (typeof _successCallback === "function") _successCallback(result);
								}
							} else {
								if (!_successCallback) {
									// Promise
									...
									if (typeof _errorCallback === "function")
										_errorCallback(`${resStatus} - ${errorStatuses[resStatus]}`);
								}
							}
						})
						.catch((err) => {
